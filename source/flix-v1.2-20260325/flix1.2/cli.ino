...
"p <name> <value> - 设置参数\n"
"preset - 重置参数reset\n"
"mfr, mfl, mrr, mrl - 低功率测试马达 (20%, 1秒, 无桨!)\n"
"mt <motor> <power> - 自定义功率测试, motor: 0=RL 1=RR 2=FR 3=FL, power 0.05-0.5\n"
...
void doCommand(String str, bool echo = false) {
	...
	if (command == "help" || command == "motd") {
		...
		testMotor(MOTOR_FRONT_RIGHT, 0.2, 1.0);
	} else if (command == "mfl") {
		testMotor(MOTOR_FRONT_LEFT, 0.2, 1.0);
	} else if (command == "mrr") {
		testMotor(MOTOR_REAR_RIGHT, 0.2, 1.0);
	} else if (command == "mrl") {
		testMotor(MOTOR_REAR_LEFT, 0.2, 1.0);
	} else if (command == "mt") {
		int motor = arg0.isEmpty() ? -1 : arg0.toInt();
		float power = arg1.isEmpty() ? 0.2 : arg1.toFloat();
		if (motor < 0 || motor > 3) {
			print("Usage: mt <motor> <power>, motor: 0=RL 1=RR 2=FR 3=FL, power 0.05-0.5\n");
		} else {
			testMotor(motor, power, 1.0);
		}
	} else if (command == "sys") {
#ifdef ESP32
		print("Chip: %s\n", ESP.getChipModel());
