...
void testMotor(int n, float power, float duration) {
	power = constrain(power, 0.05, 0.5); // 低功率上限 50%
	duration = constrain(duration, 0.5, 2.0); // 短时间，上限 2 秒
	print("Testing motor %d at %.0f%% power for %.1f s\n", n, power * 100, duration);
	motors[n] = power;
